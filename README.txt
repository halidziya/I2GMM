...
Description: I2GMM is non parametric bayesian model for clustering applications. It allows arbitrary number of clusters with arbitrary shapes. Two layer Dirichlet process lie behind this algorithm. Modeling each cluster with an Dirichlet Process with Normal distributions leads to a flexible model. Emprical evaluation on public datasets shows the effectiveness of the model. This model is also related with the ASPIRE model which works on grouped clustering problems. For more details about ASPIRE please refer to ASPIRE web site
...
Limitations : Due to covariance matrix of the gaussian distributions this algorithm does not scale well with the dimensions. Maximum dimensions used in experiments was 30. Largest size for the datasets was ? points. 
